...
Seems like Africa trends lower than everyone else while Europe and the Americas trend higher with Asia being more in the middle.
...
Changing the order put the points behind the lines so R must use the order that they are called to layer them.
...
```{r}
ggplot(data = gapminder, mapping = aes(x = gdpPercap, y = lifeExp)) +
  geom_point(alpha = 0.5, color = 'red4', size = 0.5 ) + 
  geom_smooth(method="lm", linewidth=1.5)+
  scale_x_log10() 
```
